...
import { Plus, ArrowLeft, ChevronLeft, ChevronRight, Calendar, TrendingUp, TrendingDown, Loader2, BarChart3 } from "lucide-react"
            ...
            <div className="flex flex-col gap-6">
                    ...
                    <div className="flex items-center gap-3">
                        {viewMode === 'detail' && (
                            <button 
                                ...
                                className="p-2 hover:bg-gray-200 dark:hover:bg-neutral-800 rounded-full transition-colors font-bold"
                            >
                        ...
                        <div>
                            <h2 className="text-2xl font-bold text-gray-800 dark:text-white flex items-center gap-2">
                                <BarChart3 className="text-[#5c9a38]" />
                                {viewMode === 'list' ? `Báo cáo Thu Chi ${selectedYear}` : `Chi tiết: ${getMonthName(selectedMonth!)} / ${selectedYear}`}
                            </h2>
                            <p className="text-sm text-gray-500">
                                {viewMode === 'list' ? "Tổng quan biến động tài chính theo năm" : "Danh sách các khoản thu chi thực tế trong tháng"}
                            </p>
                        </div>
                    </div>
                ...
                {!isLoading && viewMode === 'list' && (
                    <div className="grid grid-cols-1 gap-6">
                        {/* Summary Chart Card */}
                        <div className="bg-white dark:bg-neutral-900 rounded-xl border border-gray-200 dark:border-neutral-800 shadow-sm p-6 overflow-hidden">
                            <div className="flex justify-between items-center mb-6">
                                <h3 className="text-lg font-bold text-gray-800 dark:text-white whitespace-nowrap">Biểu đồ so sánh Thu - Chi</h3>
                                <div className="flex gap-4">
                                     <div className="flex items-center gap-2">
                                        <div className="w-3 h-3 rounded-full bg-green-500" />
                                        <span className="text-xs text-gray-500 font-medium">Tổng Thu</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                            ...
                            <div className="h-[300px] w-full">
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={statsByMonth} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                                        <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f0f0f0" />
                                        <XAxis dataKey="monthName" axisLine={false} tickLine={false} tick={{ fontSize: 12, fontWeight: 'bold' }} />
                                        <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 10 }} tickFormatter={(val) => `${val/1000000}tr`} />
                                        <Tooltip 
                                ...
                                >
                                    <div className="p-4 flex flex-col gap-3">
                                        <div className="flex justify-between items-start">
                                            <div className="p-2 bg-[#5c9a38]/10 rounded-lg">
                                                <Calendar className="text-[#5c9a38]" size={20} />
                                            </div>
                                        ...
                                        <div className="space-y-2 pt-2 border-t border-gray-100 dark:border-neutral-800">
                                            <div className="flex justify-between items-center text-sm">
                                                ...
                                                <span className="text-gray-700 dark:text-gray-300 font-bold">{m.income > 0 ? vnd(m.income) : "—"}</span>
                                            </div>
                                            <div className="flex justify-between items-center text-sm">
                                                <div className="flex items-center gap-1.5 text-red-600 dark:text-red-400 font-medium">
                                                    <TrendingDown size={14} /> Chi
                                                </div>
                                                <span className="text-gray-700 dark:text-gray-300 font-bold">{m.expense > 0 ? vnd(m.expense) : "—"}</span>
                                            </div>
                                        </div>

                                        <button className="mt-2 w-full py-2 text-sm font-semibold text-[#5c9a38] bg-[#5c9a38]/5 dark:bg-[#5c9a38]/10 rounded-lg group-hover:bg-[#5c9a38] group-hover:text-white transition-all">
                                            Xem chi tiết
                                        </button>
                                    </div>
                                </div>
                             ...
                             <table className="w-full text-sm text-left border-collapse">
                                <thead className="bg-gray-50 dark:bg-neutral-800 text-gray-600 dark:text-gray-300 font-semibold border-b border-gray-200 dark:border-neutral-700">
                                    <tr>
                                        ...
                                        <th className="px-6 py-4">Diễn giải / Tên khoản</th>
                                        ...
                                        filteredCategories.map((c) => (
                                            <tr key={c.id} className="hover:bg-gray-50/50 dark:hover:bg-neutral-800/50 transition-colors">
                                                ...
                                                <td className="px-6 py-4 text-center">
                                                    <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold ${c.type === "Thu" ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400"}`}>
                                                ...
                                                <td className="px-6 py-4 flex items-center justify-center gap-2">
                                                    <button 
                                                        ...
                                                        className="p-1.5 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded transition-colors font-bold text-xs"
                                                    >
                                                        Sửa
                                                    </button>
                                                    <button 
                                                        onClick={() => handleDeleteClick(c)}
                                                        className="p-1.5 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded transition-colors font-bold text-xs"
                                                    >
                                                        Xóa
                                                    </button>
                                                </td>
