    ...
    return (
        <div className="flex flex-col h-full bg-white dark:bg-neutral-900 overflow-hidden">
            ...
            {/* ── FOOTER SUB-SECTION (Totals) ── */}
                        {items.length === 0 ? (
                            <tr>
                                <td colSpan={15} className="py-12 sm:py-24 text-center">
                            ...
                            items.map((item) => (
                                <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-neutral-800/20 text-gray-600 dark:text-gray-300 group">
                                    <td className="px-2 sm:px-3 py-3 border-r border-gray-100 dark:border-neutral-800 font-mono text-[10px] sm:text-xs">{item.code}</td>
                                    <td className="px-2 sm:px-3 py-3 border-r border-gray-100 dark:border-neutral-800 min-w-[200px]">
                                        <div className="flex flex-col gap-0.5">
                                            <span className="font-bold text-gray-800 dark:text-gray-200 truncate max-w-[250px] whitespace-normal leading-tight">
                                                {item.name}
                                            </span>
                                        </div>
                                    </td>
                                    <td className="px-2 sm:px-3 py-3 border-r border-gray-100 dark:border-neutral-800 text-center">{item.unit}</td>
                                    <td className="px-2 sm:px-3 py-3 border-r border-gray-100 dark:border-neutral-800 uppercase">
                                        {isEditing ? (
                                            <input
                                                type="text"
                                                className="w-full bg-blue-50/50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 px-1 py-1 rounded text-center outline-none focus:bg-white dark:focus:bg-neutral-900 text-[10px] sm:text-xs font-semibold transition-all"
                                                value={item.batchNumber}
                                                onChange={(e) => updateItemField(item.id || "", 'batchNumber', e.target.value)}
                                            />
                                        ) : <span className="font-mono">{item.batchNumber}</span>}
                                    </td>
                                    <td className="px-2 sm:px-3 py-3 border-r border-gray-100 dark:border-neutral-800 text-center font-mono">
                                        {isEditing ? (
                                            <input
                                                type="text"
                                                className="w-full bg-blue-50/50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 px-1 py-1 rounded text-center outline-none focus:bg-white dark:focus:bg-neutral-900 text-[10px] sm:text-xs font-semibold transition-all"
                                    ...
                                    <td className="px-2 sm:px-3 py-3 border-r border-gray-100 dark:border-neutral-800 text-right">
                                        {isEditing ? (
                                            <NumericInput
                                                className="w-16 bg-blue-50/50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 px-1 py-1 rounded text-right outline-none focus:ring-2 focus:ring-blue-500/20 font-black text-blue-600 dark:text-blue-400 transition-all text-xs sm:text-sm"
                                                value={Number(item.quantity)}
                                                onChange={(v) => updateItemField(item.id || "", 'quantity', v)}
                                            />
                                    ...
                                    <td className="px-2 sm:px-3 py-3 border-r border-gray-100 dark:border-neutral-800 text-right">
                                        {isEditing ? (
                                            <NumericInput
                                                className="w-24 bg-blue-50/50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 px-1 py-1 rounded text-right outline-none focus:ring-2 focus:ring-blue-500/20 font-black text-blue-600 dark:text-blue-400 transition-all text-xs sm:text-sm"
                                                value={Number(item.importPrice)}
                                                onChange={(v) => updateItemField(item.id || "", 'importPrice', v)}
                                            />
                                        ) : <span className="text-gray-400 italic">{vnd(item.importPrice)}</span>}
                                    </td>
                                    <td className="px-2 sm:px-3 py-3 border-r border-gray-100 dark:border-neutral-800 text-right hidden md:table-cell">
                                        {isEditing ? (
                                            <NumericInput
                                                className="w-24 bg-transparent border border-transparent hover:border-gray-300 dark:hover:border-neutral-700 px-1 py-1 rounded text-right outline-none focus:bg-white dark:focus:bg-neutral-900 transition-all text-xs"
                                                value={Number(item.retailPrice)}
                                                onChange={(v) => updateItemField(item.id || "", 'retailPrice', v)}
                                            />
                                        ) : vnd(item.retailPrice)}
                                    </td>
                                    <td className="px-2 sm:px-3 py-3 border-r border-gray-100 dark:border-neutral-800 text-right font-medium text-gray-700 dark:text-gray-200">{vnd(item.totalAmount)}</td>
                                    <td className="px-2 sm:px-3 py-3 border-r border-gray-100 dark:border-neutral-800 text-right hidden lg:table-cell">
                                        {isEditing ? (
                                            <NumericInput
                                                className="w-12 bg-transparent border border-transparent hover:border-gray-300 px-1 rounded text-right outline-none text-xs"
                                                value={Number(item.discountPercent)}
                                                onChange={(v) => updateItemField(item.id || "", 'discountPercent', v)}
                                            />
                                        ) : Number(item.discountPercent ?? 0).toFixed(2).replace('.', ',')}
                                    </td>
                                    <td className="px-2 sm:px-3 py-3 border-r border-gray-100 dark:border-neutral-800 text-right hidden lg:table-cell">{item.discountAmount > 0 ? vnd(item.discountAmount) : 0}</td>
                                    <td className="px-2 sm:px-3 py-3 border-r border-gray-100 dark:border-neutral-800 text-right hidden xl:table-cell">
                                        {isEditing ? (
                                            <NumericInput
                                                className="w-12 bg-transparent border border-transparent hover:border-gray-300 px-1 rounded text-right outline-none text-xs"
                                                value={Number(item.vatPercent)}
                                    ...
                                    <td className="px-2 sm:px-3 py-3 border-r border-gray-100 dark:border-neutral-800 text-right font-black text-gray-800 dark:text-gray-100">{vnd(item.remainingAmount)}</td>
                                    <td className="px-2 sm:px-3 py-3 border-r border-gray-100 dark:border-neutral-800 text-center hidden 2xl:table-cell">
                                        {isEditing ? (
                                            <input
                                                type="text"
                                                className="w-full bg-blue-50/10 border border-transparent hover:border-gray-300 px-1 rounded outline-none text-center text-xs"
                                                value={item.registrationNumber}
                                                onChange={(e) => updateItemField(item.id || "", 'registrationNumber', e.target.value)}
                                            />
                                        ) : item.registrationNumber}
