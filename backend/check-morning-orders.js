...
async function checkMorningOrders() {
    try {
        ...
        // Lấy các đơn từ 00:00 đến 06:59:59 sáng ngày 8/7 theo giờ VN
        const slips = await ExportSlip.find({
            exportDate: {
                $gte: new Date('2026-07-08T00:00:00+07:00'),
                $lte: new Date('2026-07-08T06:59:59+07:00')
            },
            isDeleted: { $ne: true }
        }).sort({ exportDate: 1 });

        console.log(`📋 Các đơn hàng từ 00:00 đến 06:59 sáng ngày 08/07/2026\n`);
        ...
        if (slips.length === 0) {
            ...
            console.log('═'.repeat(90));
            console.log(`📊 Tổng cộng ${slips.length} đơn hàng bị sót/lệch ngày:`);
